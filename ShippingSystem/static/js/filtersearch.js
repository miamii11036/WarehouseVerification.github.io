$(document).ready(function() { //當網頁載入完成時
    ...
    function loadOrders(page) {
        ...
        $.get(`/Filtersearch/?page=${page}&year=${$('#year').val()}&month=${$('#month').val()}&region=${$('#region').val()}`)  //這一大段一定要!要不然資料庫的所有資料都會呈現
            ...
            $(".expand-btn").on("click", function(){
                ...
                if (orderdetail.is(":visible")) { 
                    ...
                    if (orderdetail.find(".order-detail-data").children().length === 0) { //如果沒有載入過
                        $.get(`/order_detail/${orderid}/`)
                            .done(function(data) { //接收views.py中order_detail函數丟過來的JsonResponse資料
                        ...
                        .fail(function(error) {
                            ...
                            window.alert("Failed to load order_detail data");
                        });
                    }
        ...
        .fail(function(error) { //如果沒有成功接收views.py的orderlist函數丟過來的JsonResponse
            console.error("Error:", error);
            const errorMessage = error.status === 404 ? "Data not found" : "Failed to load order_detail data";
